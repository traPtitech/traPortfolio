...
func (repo *ContestRepository) UpdateContest(id uuid.UUID, changes map[string]interface{}) error {
	...
	err := repo.h.Transaction(func(tx database.SQLHandler) error {
		if err := tx.Where(&model.Contest{ID: id}).First(&old).Error(); err != nil {
...
func (repo *ContestRepository) DeleteContest(id uuid.UUID) error {
	err := repo.h.Transaction(func(tx database.SQLHandler) error {
		if err := repo.h.Where(&model.Contest{ID: id}).First(&model.Contest{}).Error(); err != nil {
			return convertError(err)
		}

		if err := tx.Where(&model.Contest{ID: id}).Delete(&model.Contest{}).Error(); err != nil {
			return convertError(err)
		}
...
func (repo *ContestRepository) GetContestTeams(contestID uuid.UUID) ([]*domain.ContestTeam, error) {
	...
	err := repo.h.Model(&model.ContestTeam{}).Where(&model.ContestTeam{ContestID: contestID}).Find(&teams).Error()
	if err != nil {
		return nil, convertError(err)
	}
...
func (repo *ContestRepository) GetContestTeam(contestID uuid.UUID, teamID uuid.UUID) (*domain.ContestTeamDetail, error) {
	...
	if err := repo.h.Where(&model.ContestTeam{ID: teamID, ContestID: contestID}).First(&team).Error(); err != nil {
		return nil, convertError(err)
	}
...
func (repo *ContestRepository) UpdateContestTeam(teamID uuid.UUID, changes map[string]interface{}) error {
	...
	err := repo.h.Transaction(func(tx database.SQLHandler) error {
		if err := tx.Where(&model.ContestTeam{ID: teamID}).First(&old).Error(); err != nil {
			return convertError(err)
		}
		if err := tx.Model(&old).Updates(changes).Error(); err != nil {
			return convertError(err)
...
func (repo *ContestRepository) DeleteContestTeam(contestID uuid.UUID, teamID uuid.UUID) error {
	err := repo.h.Where(&model.ContestTeam{ID: teamID}).First(&model.ContestTeam{}).Error()
	if err != nil {
		return convertError(err)
	}

	err = repo.h.Transaction(func(tx database.SQLHandler) error {
		err = tx.Where(&model.ContestTeam{ID: teamID}).Delete(&model.ContestTeam{}).Error()
		if err != nil {
			return convertError(err)
		}
		return nil
	...
	err = repo.h.Transaction(func(tx database.SQLHandler) error {
		for _, memberID := range members {
			if _, ok := belongings[memberID]; ok {
				err = tx.Where(&model.ContestTeamUserBelonging{TeamID: teamID, UserID: memberID}).Delete(&model.ContestTeamUserBelonging{}).Error()
				if err != nil {
					return convertError(err)
				}
			}
